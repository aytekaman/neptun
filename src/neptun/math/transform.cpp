...
Ray Transform::transform_ray(const Ray& ray) const
{   
    glm::vec3 origin    = transform_point(ray.origin);
    const glm::vec3 dir = transform_vector(ray.dir);
    float tMax = ray.tMax;
    Ray tr;
    tr.origin = origin;
    tr.dir = dir;
...
Ray Transform::inv_transform_ray(const Ray& ray) const
{
    glm::vec3 origin    = inv_transform_point(ray.origin);
    const glm::vec3 dir = inv_transform_vector(ray.dir);
    float tMax = ray.tMax;
    Ray tr;
    tr.origin = origin;
    tr.dir = dir;
