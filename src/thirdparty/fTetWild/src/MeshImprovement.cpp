...
typedef std::pair<std::array<double, 3>, int> CellType;
using std::vector;

#define USE_FWN true

bool sortByX(std::array<double, 3> a, std::array<double, 3> b)
{
    return (a[0] < b[0]);
}
bool sortByY(std::array<double, 3> a, std::array<double, 3> b)
{
    return (a[1] < b[1]);
}
bool sortByZ(std::array<double, 3> a, std::array<double, 3> b)
{
    return (a[2] < b[2]);
}
...
int to_real_t_index(int t, std::vector<floatTetWild::MeshTet> ts) {
    int k = 0;
...
int to_real_v_index(int v, std::vector<floatTetWild::MeshVertex> vs) {
    int k = 0;
...
//    C.conservativeResize(index, 3);
    ...
    std::vector<MeshVertex> area_tet_vertices = mesh.tet_vertices;
    std::vector<MeshTet> area_tets = mesh.tets;
    ...
    //mesh.tet_vertices = area_tet_vertices;
    //mesh.tets = area_tets;

    // I have prev and currs
    std::ofstream output_mesh("torus_output.txt");
    std::ofstream input_mesh("torus_input.txt");

    int v_cnt = 0;
    int t_cnt = 0;
    for (auto& v : area_tet_vertices) {
        if (!v.is_removed) v_cnt++;
    }
    for (auto& t : area_tets) {
        if (!t.is_removed) t_cnt++;
    }


    std::vector<int> t_map(area_tets.size(), 0);
    int k = 0;
    for (int i = 0; i < area_tets.size(); i++) {
        if (area_tets[i].is_removed) {
            continue;
        }
        t_map[i] = k;
        k++;
    ...
    std::vector<int> v_map(area_tet_vertices.size(), 0);
    k = 0;
    int sample;
    for (int i = 0; i < area_tet_vertices.size(); i++) {
        if (area_tet_vertices[i].is_removed) {
            continue;
        }
        sample = i;
        v_map[i] = k;
    ...
    std::vector<double> maxs{ area_tet_vertices[sample][0], area_tet_vertices[sample][1], area_tet_vertices[sample][2] };
    std::vector<double> mins = maxs;

    output_mesh << "Vertices #" << v_cnt << std::endl;
    for (auto &v : area_tet_vertices) {
        if (v.is_removed) continue;
        for (int j = 0; j < 3; j++) {
            maxs[j] = std::max(v[j], maxs[j]);
            mins[j] = std::min(v[j], mins[j]);
        }
        output_mesh << v[0] << " " << v[1] << " " << v[2] << std::endl;
        input_mesh << "v " << v[0] << " " << v[1] << " " << v[2] << std::endl;
    }
    std::cout << maxs[0] << " " << maxs[1] << " " << maxs[2] << std::endl;
    std::cout << mins[0] << " " << mins[1] << " " << mins[2] << std::endl;
    output_mesh << "Tetrahedras #" << t_cnt << std::endl;
    for (auto &t : area_tets) {
        if (t.is_removed) continue;
        output_mesh << v_map[t[1]] << " " << v_map[t[0]] << " " << v_map[t[2]] 
            << " " << v_map[t[3]] << std::endl;
    }
    ...
    const static std::vector<int> NN{ A, B, K };
    vector<vector<vector<vector<CellType>>>>
        grid(A + 1,
            vector<vector<vector<CellType>>>(B + 1,
                vector<vector<CellType>>(K + 1,
                    vector<CellType>())));

    for (int t = 0; t < area_tets.size(); t++) {
        auto &tet = area_tets[t];
        if (tet.is_removed) continue;

        std::vector<int> v_ids{ tet[0], tet[1], tet[2], tet[3] };
        std::vector<std::array<double, 3>> vertices;
        vertices.push_back({ area_tet_vertices[v_ids[0]][0], area_tet_vertices[v_ids[0]][1], area_tet_vertices[v_ids[0]][2] });
        vertices.push_back({ area_tet_vertices[v_ids[1]][0], area_tet_vertices[v_ids[1]][1], area_tet_vertices[v_ids[1]][2] });
        vertices.push_back({ area_tet_vertices[v_ids[2]][0], area_tet_vertices[v_ids[2]][1], area_tet_vertices[v_ids[2]][2] });
        vertices.push_back({ area_tet_vertices[v_ids[3]][0], area_tet_vertices[v_ids[3]][1], area_tet_vertices[v_ids[3]][2] });

        std::vector<std::vector<std::array<double, 3>>> tmp{ { vertices[1], vertices[2], vertices[3]},
                                       {vertices[0], vertices[2], vertices[3]},
                                       {vertices[0], vertices[1], vertices[3]},
                                       {vertices[0], vertices[1], vertices[2]} };
        vector<int> volume_sign_order{ 1, 0, 2, 3 };

        for (int i : volume_sign_order) {
            // For each face of the tetra
            std::vector<std::array<double, 3>> face = tmp[i];

            std::array<double, 3> centroid{ 0.0, 0.0, 0.0 };

            vector<vector<std::array<double, 3>>> sorted{ {face[0], face[1], face[2]},
                                                          {face[0], face[1], face[2]},
                                                          {face[0], face[1], face[2]} };
            std::sort(sorted[0].begin(), sorted[0].end(), sortByX);
            std::sort(sorted[1].begin(), sorted[1].end(), sortByY);
            std::sort(sorted[2].begin(), sorted[2].end(), sortByZ);

            /*for (int j = 0; j < 3; j++) {
                // For each vertex of the face
                std::array<double, 3> vertex = face[j];
                for (int k = 0; k < 3; k++) {
                    centroid[k] += sorted[k][j][k];
                }
            }*/
            ...
            std::array<int, 3> cell_i;
            for (int k = 0; k < 3; k++) {
                // For each axis of the vertex
                double v = centroid[k];
                cell_i[k] = (v - mins[k]) / (maxs[k] - mins[k]) * NN[k];
            }
            grid[cell_i[0]][cell_i[1]][cell_i[2]].push_back({ centroid, t });

        }
    }
    // End init grid

    output_mesh << "Neighbours" << std::endl;

    vector<std::array<int, 4>> neighbour_list(area_tets.size(), { -1, -1, -1, -1 });
    ...
    for (int t = 0; t < area_tets.size(); t++) {
        ...
        for (int i : volume_sign_order) {
            ...
            vector<CellType> x = grid[cell_i[0]][cell_i[1]][cell_i[2]];
            ...
            for (CellType c : x) {
                ...
                bool is_equal = true;
                for (int z = 0; z < 3; z++) {
                    //if (abs(c.first[z] - centroid[z]) >= 100 * std::numeric_limits<double>::epsilon()) {
                    if(c.first[z] != centroid[z]){
                        is_equal = false;
                    }
                }
                if (is_equal) {
                    ...
                    if (flag) {
                        std::cout << "Eyvah two matches" << std::endl;
                        break;
                    }
                    output_mesh << t_map[c.second] << " ";
                    neighbour_list[t][i] = c.second;
                    flag = true;
                }
            }
        ...
        output_mesh << std::endl;
    }


    output_mesh << "Constrained Faces" << std::endl;
    int face_sum = 0;
    std::stringstream ss;

    vector<std::array<int, 4>> face_list(area_tets.size(), { -1, -1, -1, -1 });
    int cnt = 0;

    for (int k = 0; k < 1; k++) {
        for (int t_i = 0; t_i < area_tets.size(); t_i++) {
            auto t = area_tets[t_i];
            auto t2 = mesh.tets[t_i];
            if (t.is_removed) continue;

            if (k == 0) {
                if (t2.is_removed) continue;
            }
            else { // Else part is redundant but will be kept for investigation mentioned below
                if (!t2.is_removed) continue;
            }
            ...
            for (int i : volume_sign_order) {
                if (t.is_surface_fs[i] == NOT_SURFACE) {
                    //output_mesh << "- ";
                }
                else {
                    vector<int> indices;
                    ...
                    if (!t2.is_removed) {
                        ss << "f " << b << "//" << b << " " << a << "//" << a << " " << c << "//" << c << std::endl;

                        face_list[t_i][i] = face_sum;
                        face_sum++;

                        int cross_id = neighbour_list[t_i][i];

                        if (cross_id != -1) {
                            int target_face = -1;

                            for (int g = 0; g < 4; g++) {
                                if (neighbour_list[cross_id][g] == t_i) {
                                    target_face = g;
                                }
                            }

                            face_list[cross_id][target_face] = face_sum - 1;
                        }


                    }
                    else {
                        // Further investigate why this does not work
                        /*int n_id = neighbour_list[t_i][i];

                        int target_face = -1;

                        for (int g = 0; g < 4; g++) {
                            if (neighbour_list[n_id][g] == t_i) {
                                target_face = g;
                            }
                        }
                        
                        face_list[t_i][i] = face_list[n_id][target_face];
                        if(face_list[t_i][i] == -1)
                            cnt++;*/
                    }
                }
            }
    ...
    /*for (int i = 0; i < area_tets.size(); i++) {
        ...
        for (int j = 0; j < 4; j++) {
            auto v = area_tet_vertices[t.indices[j]];
            std::vector<int> n = area_tet_vertices[t.indices[(j + 1) % 4]].conn_tets;

            for (int k = 0; k < 4; k++) {
                if (j != k) {
                    auto v_other = area_tet_vertices[t.indices[k]];
                    set_intersection(std::vector<int>(v_other.conn_tets), n, n);
                }
            }
            if (n.size() == 1 && !area_tets[n[0]].is_removed) {
                output_mesh << t_map[n[0]] << " ";
            }
            else {
                int cnt = 0;
                for (auto &g : v.conn_tets) {
                    if (!area_tets[g].is_removed) cnt++;
                }
                output_mesh << cnt  << " "; // Attentinonn
            }
        }
